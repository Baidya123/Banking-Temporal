...
public class TemporalClientRunner implements ApplicationRunner {
	...
	public final Object processTransaction(TransferRequest transactionRequest) {
		...
		/*
		 *setTaskQueue :: Task queue to use for workflow tasks. It should match a task queue specified when creating a
		 * {@link io.temporal.worker.Worker} that hosts the workflow code.
		 * 
		 * setWorkflowId :: A WorkflowId prevents this it from having duplicate instances, remove it to duplicate.
		 * 
		 * setWorkflowExecutionTimeout :: The time after which workflow execution (which includes run retries and continue as new) is
		...
		// WorkflowClient.start :: Returns as soon as the workflow is scheduled to start on the server.
